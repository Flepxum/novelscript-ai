...
func logStartupConfig(cfg config.Config) {
	...
	log.Printf(
		"LLM config: provider=%s base_url=%s model=%s api_key_configured=%t org_configured=%t project_configured=%t timeout=%ds retries=%d structure_mode=%s agent_pipeline=%s chapter_batch_size=%d scene_batch_size=%d fast_path_max_chars=%d prompt_version=%s",
		...
		cfg.ModelChapterAnalysisBatchSize,
		cfg.ModelSceneExpansionBatchSize,
		cfg.ModelFastPathMaxChars,
