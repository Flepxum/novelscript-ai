...
- Go：主要后端语言。
- Gin：HTTP 路由和中间件。
- MemoryRepository：比赛阶段的内存项目、来源、任务和版本存储。
...
| 模块 | 职责 |
| --- | --- |
| `config` | 读取端口、存储路径、Schema 路径、模型 base URL、Key 和模型名 |
...
| `repository` | 内存仓库读写，集中保存项目、来源、任务和版本 |
...
`handler` 不直接访问存储仓库或模型 provider。`service` 是唯一的业务编排层，避免逻辑散落在 HTTP 层。
...
这些对象对应内存仓库中的核心数据，也对应前后端传输结构。后续接入持久化存储时可映射为数据库表。
...
- `MODEL_AGENT_PIPELINE=multi_agent` 为默认生产模式：先智能切章，再逐章分析，再建立故事圣经，再规划场景卡，再逐场扩写，最后组装和校验。
...
当前比赛可运行版本使用 `MemoryRepository`，原因是 demo 环境无需数据库初始化，评委拉取后能直接运行。内存仓库集中保存：
...
每次生成、保存、恢复或局部重写都创建新版本，不覆盖旧版本。这样 demo 时可以展示“AI 初稿 -> 人工编辑 -> 新版本 -> 历史恢复”的完整过程。

后续如需 SQLite 或 PostgreSQL，只需要在 `repository` 边界实现同一组保存、读取、版本查询方法，不需要改动 handler、service、parser、ai、validator 或前端。
