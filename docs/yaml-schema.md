...
后端和前端使用同一份 Schema：后端负责最终校验，前端负责编辑器提示和保存前预校验。YAML 会先被解析成对象，再套用 JSON Schema 校验。
...
`dialogues` 中每条对白必须包含非空 `speaker` 和非空 `line`。如果模型返回空对白占位，后端会在生成归一化阶段清理并补齐可编辑对白，保存 YAML 时也会拒绝空台词。
...
JSON Schema 适合检查字段类型、必填项、枚举值和数组长度，但不适合完整表达跨数组引用关系。因此系统会把校验拆成两层：

- Schema 校验：检查 `schema_version`、根字段、字段类型、章节数量不少于 3。
- 业务校验：检查角色引用、场景归属、章节编号是否真实存在、ID 是否重复。

这样设计的原因是保持 Schema 简洁稳定，同时把更依赖数据库和上下文的规则放在 Go 后端里执行。

## 9. 版本演进

当前版本为 `1.0`。后续如果加入分集、镜头、拍摄预算、分镜等字段，应通过以下方式升级：
